...
def full_leadership_rating(resume_dir):
    ...
    for pdf_path in sorted(resume_dir.glob("*.pdf")):
        ...
        leadership_buckets = [
            exp.leadership_bucket for exp in result.experiences]
        leadership_weighted = [bucket_weight(
            bucket) for bucket in leadership_buckets]
        leadership_weighted_total = diminishing_sum(leadership_weighted)
        ratings[pdf_path.name] = (lea_rating(leadership_weighted_total), leadership_weighted_total)
    return ratings
...
if __name__ == "__main__":
    ...
    print("\n--- Leadership Ratings ---")
    for resume_name, (rating, leadership_weighted_total) in results.items():
        print(f"{resume_name}: {rating}, {leadership_weighted_total}")
